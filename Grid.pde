public class Grid {
  ...
  public void removeSet() {
    ...
    if(cardsInPlay > 12 || deck.size() == 0){
      ...
      ArrayList<Location> lastThreeCards = new ArrayList<Location>();
      
      lastThreeCards.add(new Location(cardsInPlay / 3, cardsInPlay % 3));
      lastThreeCards.add(new Location(cardsInPlay - 1 / 3, (cardsInPlay-1) % 3 ));
      lastThreeCards.add(new Location(cardsInPlay - 2 / 3, (cardsInPlay-2) % 3));
      
      for(Location currentLoc : selectedLocs){
        for(Location currentCard : lastThreeCards){
          if(currentLoc.equals(currentCard)){
            selectedLocs.remove(selectedLocs.indexOf(currentLoc));
            lastThreeCards.remove(lastThreeCards.indexOf(currentCard));
          }
        }
      }
      
      for(Location currentLoc : selectedLocs){
        int cardCurrentIndex = selectedLocs.indexOf(currentLoc);
        Location currentCard = lastThreeCards.get(cardCurrentIndex);
        board[currentLoc.getCol()][currentLoc.getRow()] = board[currentCard.getCol()][currentCard.getRow()];
      }
    } else if(cardsInPlay == 12 && deck.size() != 0){
      
      for(Location currentLoc : selectedLocs){
        board[currentLoc.getCol()][currentLoc.getRow()] = deck.deal();
      }
    }
  }
  ...
  public void addCardToBoard(Card card) {
    ...
    int row = cardsInPlay / 3; //how many rows have been occupied
    int col = cardsInPlay % 3; //gives us how much of an column is occupied
  ...
  public void addColumn() {
    ...
    if(deck.size() == 0){ //deck has no cards left
      ...
      score += 5; //increments players score
      addCardToBoard(deck.deal()); //adds three cards to the board
