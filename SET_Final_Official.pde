...
public void newGame() {
  ...
  deck = new Deck(); //initalizes the graphics/functionality
  grid = new Grid();
  
  message = 0; //tells what message to display on the board
  score = 0; //sets score
  state = State.PLAYING;
  currentCols = 4; //only four columns should show up
  
  for(int i = 0; i<(currentCols * ROWS); i++){
    grid.addCardToBoard(deck.deal());
  }
  
  timeElapsed = 0; //start for a new game
